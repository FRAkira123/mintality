...
bool    ImGui_Init(void* hwnd, IDirect3DDevice9* device)
{
	...
	io.RenderDrawListsFn = ImGui_DrawData;   // Alternatively you can set this to NULL and call ImGui::GetDrawData() after ImGui::Render() to get the same ImDrawData pointer.
	io.ImeWindowHandle = g_hWnd;

	io.Fonts->AddFontFromFileTTF(XorStr("C:\\Windows\\Fonts\\Tahoma.ttf"), 11.0f, NULL, io.Fonts->GetGlyphRangesCyrillic());
	...
	style.Colors[ImGuiCol_CheckMark] = ImVec4(0.78f, 0.31f, 0.31f, 1.00f);
...
void ImGui_NewFrame()
{
	...
	io.MouseDrawCursor = bShowWindow;
...
LRESULT WINAPI myWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		bShowWindow ^= 1; 
		std::string msg = XorStr("cl_mouseenable ") + std::to_string(!bShowWindow);
		pEngine->ClientCmd_Unrestricted(msg.c_str());
		if (!graph)
		{
			graph = true;
			pEngine->ClientCmd_Unrestricted(XorStr("net_graph 1"));
			//pEngine->ClientCmd_Unrestricted(XorStr("map de_dust2"));
