    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar))
				window->DrawList->AddRectFilledMultiColor(title_bar_rect.GetTL(), title_bar_rect.GetBR(), window->Color(ImGuiCol_TitleBgActive), window->Color(ImGuiCol_TitleBgActive), window->Color(ImGuiCol_WindowBg), window->Color(ImGuiCol_WindowBg));
...
static bool CloseWindowButton(bool* p_opened)
{
  ...
  //  if (hovered)
 //   {
        window->DrawList->AddLine(center + ImVec2(+cross_extent,+cross_extent), center + ImVec2(-cross_extent,-cross_extent), window->Color(hovered ? ImGuiCol_Text : ImGuiCol_CloseButton));
        window->DrawList->AddLine(center + ImVec2(+cross_extent,-cross_extent), center + ImVec2(-cross_extent,+cross_extent), window->Color(hovered ? ImGuiCol_Text : ImGuiCol_CloseButton));
  //  }
...
bool ImGui::ColorButton(const ImVec4& col, bool small_height, bool outline_border)
{
	...
	ImVec4 colRGB(col);
	colRGB.w = 255;
	RenderFrame( bb.Min, bb.Max, window->Color( colRGB ), outline_border, style.FrameRounding );
	ImVec4 colAlpha;
	colAlpha.x = colAlpha.y = colAlpha.z = col.w;
	colAlpha.w = 255;
...
void ImGui::ColorPicker( const char* label, float col[4] ) 
{
